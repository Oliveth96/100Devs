...
'''
Module providing for currency conversion
'''
c = CurrencyRates()
amount = int(input("Please enter the amount: "))
from_currency = input("You are converting from what currency : ").upper()
to_currency = input('You are converting to which currency :').upper()
...
class App(tk.Tk):
    ...
    def __init__(self, converter):
        ...
        # Label
        self.intro_label = Label(self, text='Welcome to Real Time Currency Convertor by legalandcodes', fg='blue', relief=tk.RAISED,
                                 borderwidth=3)
        self.intro_label.config(font=('Courier', 15, 'bold'))

        self.date_label = Label(self,
                                text=f"1 Nigerian Naira equals = {self.currency_converter.convert('NGN', 'USD', 1)} USD \n Date : {self.currency_converter.data['date']}",
                                relief=tk.GROOVE, borderwidth=5)

        self.intro_label.place(x=50, y=5)
        self.date_label.place(x=320, y=50)
        ...
        '''
        Personalization of the dropdown bar
        '''
        ...
        self.amount_field.place(x=156, y=150)
        self.to_currency_dropdown.place(x=520, y=120)
        # self.converted_amount_field.place(x = 346, y = 150)
        self.converted_amount_field_label.place(x=530, y=150)

        # Convert button
        self.convert_button = Button(self, text="Convert", fg="black", command=self.perform)
        self.convert_button.config(font=('Courier', 10, 'bold'))
        self.convert_button.place(x=375, y=135)
